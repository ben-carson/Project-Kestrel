...
class Gateway {
  ...
  setToken(appId: string, token: string) { this.tokens.set(appId, token); }
  clearToken(appId: string) { this.tokens.delete(appId); }

  async request(appId: string, endpoint: string, options?: RequestInit) {
