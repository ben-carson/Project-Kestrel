...
import { useUIStore, TOAST_TYPES } from "./useUIStore";
    ...
    (set, get) => ({
      ...
      storeMemory: (widgetId, memoryEvent) => {
        try {
            ...
            // ✅ success toast
            useUIStore.getState().showToast("Saved insight", TOAST_TYPES.SUCCESS);
      ...
      recordFeedback: (widgetId, feedback, context, operatorId = 'default') => {
        ...
        // Toast per feedback type
        switch (feedback) {
            case FEEDBACK_TYPES.HELPFUL:
              useUIStore.getState().showToast("Thanks for the feedback!", TOAST_TYPES.SUCCESS);
            break;
            case FEEDBACK_TYPES.SNOOZE:
              useUIStore.getState().showToast("Snoozed for now", TOAST_TYPES.INFO);
            break;
            case FEEDBACK_TYPES.IGNORE:
              useUIStore.getState().showToast("Okay, we’ll dial it back", TOAST_TYPES.WARNING);
            break;
            default:
              useUIStore.getState().showToast("Feedback noted", TOAST_TYPES.INFO);
        }
       ...
       * @note When this function returns true, the consuming component
       * should inform the user with a toast, e.g.:
       * useUIStore.getState().showToast("Alert suppressed by your settings", TOAST_TYPES.INFO);
       */
      ...
      performMemoryMaintenance: (silent = false) => {
        ...
        set(produce(draft => {
          ...
          Object.keys(draft.widgetMemories).forEach(widgetId => {
            ...
            draft.widgetMemories[widgetId] = draft.widgetMemories[widgetId]
              .filter(memory => (now - (memory.timestamp || 0)) < maxAge);
          ...
          draft.feedbackHistory = draft.feedbackHistory
            .filter(feedback => (now - (feedback.timestamp || 0)) < maxAge);
          ...
          Object.keys(draft.correlationData).forEach(patternId => {
            ...
            draft.correlationData[patternId] = draft.correlationData[patternId]
              .filter(correlation => (now - (correlation.timestamp || 0)) < maxAge);
          ...
          Object.keys(draft.operatorProfiles).forEach(operatorId => {
            const profile = draft.operatorProfiles[operatorId];
            if ((now - (profile.lastActivity || 0)) > maxAge) {
        ...
        if (cleaned && !silent) {
          // ✅ PATCHED: Use showToast directly
          useUIStore.getState().showToast(
            "Old widget memories have been cleaned up",
            TOAST_TYPES.INFO
          );
        }
      ...
      getMemoryStatistics: () => {
        ...
        return {
          ...
          activeSuppressions: state.suppressionRules.filter(rule => 
            rule.active && (rule.until || 0) > Date.now()
          ).length,
       ...
       */
      resetMemoryStore: () => {
        // ✅ PATCHED: Replaced non-existent showConfirmation with window.confirm
        const confirmed = window.confirm(
          'This will permanently delete all widget memories, feedback history, and learning data. This action cannot be undone.'
        );
